package com.think360.pro.healthguru.doctors.utils;

/**
 * Created by think360 on 18/04/17.
 */

public class AppConstants {

    public static final String BASE_URL = "http://think360.in/sosimpli/api/index.php/12345/";

    public static final String DRIVER_ID = "DRIVER_ID";
    public static final String driver_id = "driver_id";
    public static final String DRIVER_NAME = "DRIVER_NAME";
    public static final String WORKER_PROFILE_IMAGE_URL = "WORKER_PROFILE_IMAGE_URL";
    public static final String DRIVER_EMAIL = "DRIVER_EMAIL";
    public static final String IS_LOGIN = "IS_LOGIN";
    public static final String IS_REMEMBER_TAPPED = "IS_REMEMBER_TAPPED";

    public static final String IMAGE_URL = "IMAGE_URL";

    public static final String PROJECT_RUNNING = "PROJECT_RUNNING";
    public static final String OPERATOR_PHONE = "OPERATOR_PHONE";

}
